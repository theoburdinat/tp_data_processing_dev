...
object Main {
  def main(args: Array[String]) {
    ...
    val climateNewsQuery = spark.sql("SELECT * FROM news WHERE containsWordGlobalWarming = true")
    climateNewsQuery.show()
    ...
    val climateNewsTyped: Dataset[News] = newsDatasets.filter(_.containsWordGlobalWarming)
    ...
    enrichedDataset.write.partitionBy("date", "media").parquet("save.parquet")
